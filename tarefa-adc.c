...
const int SPEED = 1;  // Velocidade de movimento
...
void leitura_e_controle_joystick(uint slice1, uint slice2) {
    ...
    const uint16_t DEADZONE = 170;  // Melhor valor empírico para a deadzone do joystick

    adc_select_input(0);  // Leitura do canal 1
    uint16_t y_value = adc_read();
    
    adc_select_input(1);  // Leitura do canal 2
    ...
    // Implementação do deadzone no joystick que passa para o pwm
